...
cutted_past_name = "RumexWeedsCuttedSameSize"

def cut_img_and_save(x_min, y_min, x_max, y_max, source_dir, destination_dir, w_resize, h_resize):
	img = Image.open(source_dir)
	image_arr = np.array(img)
	image_arr = image_arr[y_min:y_max, x_min:x_max]
	new_image = Image.fromarray(image_arr)
	new_image = new_image.resize((w_resize, h_resize))
...
def cut_and_save_img_from_past(past, seq, destination_folder, width_avg, height_avg, resize_w, resize_h):
	weed_counter = 0
	pasture_counter = 0
	...
	for file in arr_plants:
		...
		try:
			...
			if type(file["box"]) == list:
				...
				for box in file["box"]:
					...
					y_max = int(box["@ybr"])
					arr_of_limits_weeds.append((x_min, y_min, x_max, y_max))
					cut_img_and_save(x_min, y_min, x_max, y_max, (dir_imgs+name_img), (dir_complete + "/Weed_" + str(weed_counter) + ".jpg"), resize_w, resize_h)
					weed_counter+=1
		...
		cut_img_and_save(x_min_past, y_min_past, x_max_past, y_max_past, (dir_imgs+name_img), (dir_complete + "/Pasture_" + str(pasture_counter) + ".jpg"), resize_w, resize_h)
		pasture_counter+=1
...
# Acha a menor resolucao (width * height)
for past in past_and_last_seq: # Para todas as pastas
	for i in range(past[1]+1): # Para todas as sequencias
		res_aux, width_aux, height_aux, file_name_aux = get_min_wh_weeds(past[0], i)

		if res_aux < smallest_res:
			smallest_res = res_aux
			smallest_width = width_aux
			smallest_height = height_aux
			smallest_name = file_name_aux
			print("Past: ", past[0], "   seq: ", i)
			print(smallest_res, "-", smallest_width, "-", smallest_height, "-", smallest_name, "\n")
...
for past in past_and_last_seq: # Para todas as pastas
	for i in range(past[1]+1): # Para todas as sequencias
		...
		cut_and_save_img_from_past(past[0], i, cutted_past_name, avg_x, avg_y, smallest_width, smallest_height)